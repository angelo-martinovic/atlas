...
function [ correctPixels,totalPixels,confusionMatrixImg ] = EvaluateLabeling( dataset, ourLabeling, groundTruth, nClasses, ignoreClasses )

    if nargin~=5
        error('Usage: EvaluateLabeling( dataset, ourLabeling, groundTruth, nClasses, ignoreClasses)');
    end
    
    % HACK
    % In ECP, replace chimney prediction with roof.
    if strncmp(dataset,'haussmann',9)
        ourLabeling(ourLabeling==8)=5;
    end
