function e = objfunJoint22(boundingBoxes_win, boundingBoxes_balc, classidx, PM_win, PM_balc, win_ddw, win_ddh, balc_ddw, balc_ddh)
    ...
    for i = 1 : size(balcbb,2)
        ...
        for j = 1 : size(winbb,2)
            ...
            ww = 1;
             maxval = maxval +ww*tukey(win_ddw*2 +1, win_ddw*2);
             maxval = maxval +tukey(11, 10);
             
            e1 = e1+ tukey(balcbb(2,i) - winbb(4,j), 10);
            ...
            if ol(1)>0 
                if ol(2) >0 || (abs(balcbb(2,i) - winbb(4,j)) < balc_ddh)
                      %center balcony
                      e1 = e1 + ww*tukey(cp1-cp2, win_ddw*2);
                 
                      
                else
                    e1 = e1 + ww*tukey(win_ddw*2 +1, win_ddw*2);
