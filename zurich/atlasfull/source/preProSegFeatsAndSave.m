...
% numDetections2 = size(detections{2},1);
% detectionLabels = []; 
% detectionLabels2 = [];
...
%         if numDetections>1
%             %Average box filter over n neighbors
%             for l=1:8
%                 detectionLabels(:,l) = smooth(detectionLabels(:,l),numDetections/5);
%             end
%             
%             %Sums all of the values higher than r
...
%         figure;
%         hold on; plot(detectionLabels(:,9),detectionLabels(:,1),'Color','red')    %window
%         hold on; plot(detectionLabels(:,9),detectionLabels(:,2),'Color','green')  %wall
%         hold on; plot(detectionLabels(:,9),detectionLabels(:,3),'Color','blue')   %balcony
%         hold on; plot(detectionLabels(:,9),detectionLabels(:,4),'Color','black')  %door
%         hold on; plot(detectionLabels(:,9),detectionLabels(:,5),'Color','yellow') %roof
%         hold on; plot(detectionLabels(:,9),detectionLabels(:,6),'Color','cyan')   %sky
...
% end
% 
% numDetectionsValid = size(detections{3},1);
% numDetectionsValid2 = size(detections{4},1);
...
for i = 1:length(allData)
    disp([num2str(i) '/' num2str(length(allData))]);
...
%         end
    ...
    allData{i}.segDistributions = segDistributions;
...
onlyGoodDistributions = zeros(params.numLabels,upperBoundSegPairsNum);
startOnlyGood = 1;
...
allSegDistributions =  zeros(params.numLabels,upperBoundSegPairsNum);
startAllSegs = 1;
...
for i = 1:length(allData)
    ...
    segDistributions = allData{i}.segDistributions;   %Pixel counts
...
%     disp(size(segDistributions))
            ...
            allSegDistributions(:,startAllSegs) = segDistributions(s,:)';
...
%             for g = 1:numGood
                ...
                onlyGoodDistributions(:,startOnlyGood:startOnlyGood+numGood-1) = repmat(segDistributions(s,:)',1,numGood) + segDistributions(goodNeighbors,:)';
%             end
...
allSegDistributions= allSegDistributions(:,1:numAllSegs);
...
onlyGoodDistributions= onlyGoodDistributions(:,1:numOnlyGood);
...
save(neighName,'allData','goodPairsL','goodPairsR','badPairsL','badPairsR','meanAll','stdAll','onlyGoodL',...
    'onlyGoodR','onlyGoodLabels','onlyGoodDistributions','onlyBadL','onlyBadR','allSegs','allSegLabels','allSegDistributions',...
    '-v7.3');
