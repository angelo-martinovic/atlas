...
%         disp(objectGroupList);
       ...
       for class=[1 3 4]
           ...
           while 1
                it = it+1;
                disp(['Grid iteration ' num2str(it) '...']);
                [model,inliers,error] = RansacGrid(windowList,sgmp);
                if isempty(model)
                    disp('No more grids found.');
                    break;
                end
                disp(['Grid found, error: ' num2str(error)]);
    %             newGroup = windowList(inliers);
                groups{end+1} = struct('model',model,'group',windowList(inliers));
                % Remove inliers from the list
                windowList = windowList(~ismember(1:size(windowList,2),inliers));
           end

           it = 0;
           while 1
                it = it+1;
                disp(['Line iteration ' num2str(it) '...']);
                [model,inliers] = RansacLine(windowList,sgmp);
                if isempty(model)
                    disp('No more lines found.');
                    break;
                end
                disp('Line found.');
    %             newGroup = windowList(inliers);
           ...
           if length(windowList)>0
               groups{end+1} = struct('model',[],'group',windowList);
               disp([num2str(length(windowList)) ' object(s) left ungrouped.']);
           end
           disp('Done!');
       end
